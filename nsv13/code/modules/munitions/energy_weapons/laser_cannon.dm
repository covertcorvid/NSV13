...
/obj/machinery/ship_weapon/laser_cannon
	...
	circuit = /obj/item/circuitboard/machine/laser_cannon
...
/obj/machinery/ship_weapon/laser_cannon/set_position(obj/structure/overmap/OM)
	..()
	overlay = linked.add_weapon_overlay("/obj/weapon_overlay/laser")
 ...
 */
/obj/machinery/ship_weapon/laser_cannon/can_fire(shots = weapon_type.burst_size)
...
/obj/machinery/ship_weapon/laser_cannon/fire(atom/target, shots = weapon_type.burst_size, manual = TRUE)
	if(can_fire(shots))
		// Avoid null pointer expections
		var/dummy = new/obj/item/projectile/bullet/laser
		chambered = dummy
		ammo += dummy
		. = ..()

/*
 * Flashes people standing nearby and fires a heavy laser forwards
 */
/obj/machinery/ship_weapon/laser_cannon/local_fire(atom/target)
	for(var/mob/living/M in get_hearers_in_view(7, get_turf(src))) // Flash nearby mobs
		if(M.stat != DEAD)
 ...
 * Flashes people standing near space windows.
 */
/obj/machinery/ship_weapon/laser_cannon/overmap_fire(atom/target)
	..()
...
/obj/machinery/ship_weapon/laser_cannon/after_fire()
	. = ..()
